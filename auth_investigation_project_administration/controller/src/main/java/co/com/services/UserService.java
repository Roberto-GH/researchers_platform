...
public class UserService implements IUserService {
   ...
   @Autowired
   GeneralConfigurationManager generalConfiguration;
   ...
   @Override
   public ResponseEntity<String> singup(UserRequestDTO userRequest) throws ConstraintViolationException, Exception {
   ...
   public ResponseEntity<String> getAll() throws Exception {
      ...
      List<UserResponseDTO> userResponse = modelMapper.map(usersFound, new TypeToken<List<UserResponseDTO>>() {
      }.getType());
   ...
   @Override
   public ResponseEntity<String> validateToken(String token) throws ConstraintViolationException, Exception {
      try {
         UserEntity user = getUserByToken(token);
         if (!Objects.isNull(user)) {
            UserResponseDTO userResponse = modelMapper.map(user, new TypeToken<UserResponseDTO>() {
            }.getType());
            response = finalResponse.getResponse(userResponse, responseHeaders, HttpStatus.OK);
         } else {
